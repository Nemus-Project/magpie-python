...
sVec    = logspace(0,12,50) ;
sMat    = [sVec,sVec./sVec*sVec(end);0*sVec,sVec] ;
...
%v = VideoWriter('modes.mp4');
%open(v);

for nS = 1 : 2*length(sVec)
    ...
    [V,Df] = eigs(biHarm-0*speye((Ny+1)*(Nx+1)),9,'smallestabs') ;
    ...
    colormap('copper') ;
    for m = 1 : 9
        mdShape = reshape(V(:,m),[(Ny+1),(Nx+1)]) ;
        subplot(3,3,m)
        mesh(real(mdShape),abs(mdShape)/max(max(abs(mdShape))),'FaceColor','texturemap') ; view(2); axis equal ; axis tight ; shading interp ;
        tt = sprintf('Mode %d',m) ; title(tt) ;
